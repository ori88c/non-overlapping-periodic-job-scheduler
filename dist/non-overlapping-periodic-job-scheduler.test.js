...
const createError = (jobID) => new CustomJobError(`Job no. ${jobID} has failed`, jobID);
/**
 ...
 */
const triggerEventLoopIteration = async () => {
...
describe('NonOverlappingPeriodicJobScheduler tests', () => {
     ...
     */
    const mockComputeNextDelay = (previousExecutionMetadata) => {
        ...
        if (durationMs === index_1.NO_PREVIOUS_EXECUTION) {
    ...
    describe('Happy path tests', () => {
        test('should trigger executions as expected when all jobs succeed', async () => {
            ...
            const jobSpy = jest.fn().mockImplementation(job);
            const scheduler = new index_1.NonOverlappingPeriodicJobScheduler(jobSpy, computeNextDelaySpy);
            // Not started yet.
